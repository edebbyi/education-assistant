...
class DocumentProcessor:
    ...
    def process_pdf(self, pdf_file) -> str:
        ...
        try:
            ...
            if storage_success and self.user_id:
                ...
                db.initialize()
                try:
                    db.save_document(self.user_id, pdf_file.name, doc_hash, f"chunks: {len(text_chunks)}")
                except Exception as e:
    ...
    def get_context(self, question: str, specific_document: str = None) -> List[str]:
        ...
        if self.pc:
            try:
                ...
                return [ctx['text'] for ctx in sorted_contexts[:limit]]
