...
def init_user_specific_components():
    ...
    if 'generator' not in st.session_state or st.session_state.get('current_user_id') != user_id:
...
def generate_response(user_question: str):
    ...
    try:
        context = st.session_state.processor.get_context(
            user_question,
            specific_document=None
        )
        return st.session_state.generator.generate_response(user_question, context)
